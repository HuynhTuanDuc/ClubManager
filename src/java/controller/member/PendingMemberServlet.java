    ...
    throws ServletException, IOException {
       HttpSession session = request.getSession(false);
        ...
        String userRole = (String) session.getAttribute("role");
         if (!"ClubLeader".equals(userRole)) {
        response.sendRedirect("accessDenied.jsp");
        return;
    }

String messages = (String) session.getAttribute("messages");
String messageTypes = (String) session.getAttribute("messageTypes");

if (messages != null && messageTypes != null) {
    request.setAttribute("messages", messages);
    request.setAttribute("messageTypes", messageTypes);
    session.removeAttribute("messages");
    session.removeAttribute("messageTypes");
}
        int clubId = -1;
        try {
